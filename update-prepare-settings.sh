...
sed -i 's/marker: " {mark} {{ item.vm_name }}"/marker: "# {mark} {{ item.vm_name }}"/' updateCephCluster.yml
...
then
	...
	sed -i 's/- name: ceph-deploy mgr create/#- name: ceph-deploy mgr create/' updateCephCluster.yml
	sed -i '/shell: ceph-deploy mgr create "{{ item.vm_name }}"/{n;s/.*/      #with_items:/}' updateCephCluster.yml
	sed -i 's/shell: ceph-deploy mgr create "{{ item.vm_name }}"/#shell: ceph-deploy mgr create "{{ item.vm_name }}"/' updateCephCluster.yml

	sed -i 's/- name: ceph-deploy mon add/#- name: ceph-deploy mon add/' updateCephCluster.yml
	sed -i '/shell: ceph-deploy mon add "{{ item.vm_name }}"/{n;s/.*/      #with_items:/}' updateCephCluster.yml
	sed -i 's/shell: ceph-deploy mon add "{{ item.vm_name }}"/#shell: ceph-deploy mon add "{{ item.vm_name }}"/' updateCephCluster.yml
...
then
	...
	sed -i 's/- name: ceph-deploy osd create/#- name: ceph-deploy osd create/' updateCephCluster.yml
	sed -i '/shell: ceph-deploy osd create --data \/dev\/sdb "{{ item.vm_name }}"/{n;s/.*/      #with_items:/}' updateCephCluster.yml
	sed -i 's/shell: ceph-deploy osd create --data \/dev\/sdb "{{ item.vm_name }}"/#shell: ceph-deploy osd create --data \/dev\/sdb "{{ item.vm_name }}"/' updateCephCluster.yml

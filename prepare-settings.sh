...
# Not to set
initial_mon_name=$ceph_mons_name_begin"-0"
...
if ! test -d vars_files;
then
	mkdir vars_files
fi

if ! test -d group_vars;
then
	mkdir group_vars
fi
...
if ! test -f group_vars/proxmox.yml;
then
...
if ! test -f group_vars/cephcluster.yml;
then
...
if ! test -f inventory/ceph-cluster-inventory.yml;
then
...
do
	...
	if ! test -f host_vars/$proxmox_name.yml;
	then
...
if ! test -f host_vars/$ceph_admin_hostname.yml;
then
	echo "ansible_host: "$ceph_admin_ip >> host_vars/$ceph_admin_hostname.yml
fi

if ! test -f vars_files/ceph-admin-vars.yml;
then
...
if ! test -f vars_files/ceph-mon-vars.yml;
then
...
if ! test -f vars_files/ceph-osd-vars.yml;
then
...
if ! test -f vars_files/ceph-vars.yml;
then
	...
	echo "initial_mon_name: "$initial_mon_name >> vars_files/ceph-vars.yml
	echo "ceph_network: "$ceph_network".0/"$netmask >> vars_files/ceph-vars.yml
...
sed -i 's/operation_node_to_add/'$operation_node'/g' destroyVMS.yml

sed -i 's/operation_node_to_add/'$operation_node'/g' deleteMons.yml

sed -i 's/operation_node_to_add/'$operation_node'/g' deleteOsds.yml

echo ""
echo ""

echo "ceph_components_to_delete:" >> vars_files/ceph-to-delete-hosts-vars.yml

echo "# Name of ceph mons or osds to delete, example:" >> vars_files/ceph-to-delete-hosts-vars.yml

echo "#  - mon-1" >> vars_files/ceph-to-delete-hosts-vars.yml

echo "#  - osd-2" >> vars_files/ceph-to-delete-hosts-vars.yml
...
echo 'echo "Play ansible-playbook to remove VMs"' >> clear-config.sh
